...
import com.posprinter.printdemo.PrinterServerService
...
class MainActivity : AppCompatActivity() {
    ...
    private var pos = 0
    private var macDevice: UdpDevice ?= null
    ...
    private val netLauncher = registerForActivityResult(StartActivityForResult()) { result ->
        if (result.resultCode == RESULT_OK && result.data != null) {
            macDevice = result.data!!.getSerializableExtra(SelectNetActivity.MAC_ADDRESS) as UdpDevice
            bind.addressEt.setText(macDevice!!.macStr)
        }
    }
    ...
    private fun connectMAC() {
        if (macDevice == null) {
            ...
            App.get().connectNet(macDevice!!.ipStr)
        }
    }
